...
(defun my/vterm-switch-to-buffer ()
  "Interactively pick a vterm buffer and switch to it.
Each terminal is listed with a marker (• current, ◦ visible), its name, its dir,
and the command executing in it (if any). Non-visible terminals are listed first,
then the visible ones, and the current one is the last."
  (interactive)
  (let* ((all-bufs (my/vterm-buffers))
         (cur-buf (car (memq (current-buffer) all-bufs)))
         (bufs (remq cur-buf all-bufs))
...
(defface my/vterm-switch-dir-face '((t :inherit (italic font-lock-comment-face)))
  "Face of the terminal's dir in the terminal switcher.")

(defun my/vterm--switch-candidate-string (buf)
  "Return a single line describing vterm buffer BUF: a marker (• current, ◦ visible),
its name, dir, and running command."
  (let ((marker (cond ((eq buf (current-buffer)) "•")
                      ((get-buffer-window buf) "◦")
                      (t " ")))
        (name (format "%-15s" (my/string-truncate (buffer-name buf) 15)))
        (dir (format "%-35s" (my/shorten-path (buffer-local-value 'default-directory buf) 35)))
        (cmd (if-let* ((cmd (my/vterm--running-command buf)))
    ...
    (concat marker " "
            ...
            "  " (propertize dir 'face 'my/vterm-switch-dir-face)
